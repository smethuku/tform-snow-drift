"""
Enhanced snowflake_utils.py with stored procedure support for role grants

This version uses your custom stored procedure get_all_grants_of_roles()
to retrieve all role grants in a single efficient call.
"""
...
def get_snowflake_resources(resource: str, attributes: List[str], sql_query: str, host: str, 
    ...
    Special handling for RoleGrant resource which uses stored procedure.

    Args:
        resource (str): Name of the resource being queried (e.g., 'Warehouse', 'RoleGrant').
    ...
    if resource == "RoleGrant":
        return get_role_grants_via_procedure(
